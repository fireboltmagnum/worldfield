...
Like, imagine a student who could look at a picture of a red square, hear the words "red square," and remember seeing one before – and all three of those things would light up the same region of their brain. 
...
"Direct multimodality" just means: text and images go into the same space without being converted to one or the other first.

Right now, WorldField only handles:
- text
- images

Images get turned into vectors by a small CNN. Text gets turned into vectors by a character-level RNN. After encoding, both just become vectors in the same shared space.
...
WorldField isn't 2 models glued together. it something like this
...
Not two separate things that happen to be connected. They're in the same space.
...
The main paper is Bellec et al., "Long short-term memory and learning-to-learn in networks of spiking neurons" (NeurIPS 2018). 

It's a recurrent neural network made of spiking neurons. "Spiking" means the neurons fire discrete spike events, kind of like real neurons in your brain. LSNN neurons are adaptive—they change their excitability based on recent firing. So the network gets built-in memory just from how the neurons adapt over time.
...
WorldField is not actually implementing LSNN. We are not simulating spikes, membrane voltage, or any of that neuroscience. But in a similar vein:
...
Day 3  → Can ONE vector hold multiple concepts? (no)
...
Made 8 synthetic-coloured shapes (red squares, blue circles, etc.). Also made matching text descriptions like "a red square." Then I trained two encoders—one for images, one for text—to push both the image and its matching text toward the same spot in the latent space using contrastive loss.
...
Retrieval stayed perfect even when the memory was full of noise.
...
![alt text](slot_memory.png)
### Day 5: Graph Reasoning

The question:

I have fragments and slots. But how do concepts relate? If I've seen "cat" and "sofa" together before, should I expect them to be related?
...
Started breaking around 40% contamination. More hops didn't fix it.

The lesson:

One shot isn't enough. You need something that evolves over time.
